...
def calc_expiry_pnl(e: dict, lot_size: int) -> dict:
    ...
    # ── Slippage (from worker payload, or recompute if missing) ──
    slippage = e.get("slippage", None)
    if slippage is None:
        # Recompute: avg_fill vs ltp as mid proxy
        fill_qty   = e["qty_today_buy"] + e["qty_today_sell"]
        net_today  = e["qty_today_buy"] - e["qty_today_sell"]
        buy_val    = e["qty_today_buy"]  * (e["buy_avg"]  or 0)
        sell_val_2 = e["qty_today_sell"] * (e["sell_avg"] or 0)
        avg_fp     = ((buy_val + sell_val_2) / fill_qty) if fill_qty > 0 else None
...
def build_table(data: list[dict]) -> tuple[pd.DataFrame, dict]:
    ...
    """
    rows = []
    kpis = {"net_exp": 0.0, "gross_exp": 0.0, "carry": 0.0, "day": 0.0, "net": 0.0, "slippages": []}
...
def slip_td(v: Optional[float], open_qty: float = 0) -> str:
    """Slippage in basis points. Colour is direction-aware:
       LONG : negative bp = good (green), positive = bad (red)
       SHORT: positive bp = good (green), negative = bad (red)
    """
    if v is None:
        return '<td class="zer">—</td>'
    bps  = v * 10000
    ...
    if abs(bps) < 0.5:
        cls = "zer"
    elif open_qty > 0:   # long — negative is good
        cls = "pos" if bps < 0 else "neg"
    elif open_qty < 0:   # short — positive is good
        cls = "pos" if bps > 0 else "neg"
    else:
        cls = "zer"
...
def build_position_table_html(data: list[dict], expand_all: bool, expanded_syms: set) -> str:
    ...
    for item in data:
        ...
        # Weighted avg slippage for stock row (weight = traded_val per expiry)
...
def main():
    ...
    # Median slippage in basis points across all traded expiries
    slips = kpis.get("slippages", [])
    if slips:
        import statistics as _stats
        med_slip_bps = _stats.median(slips) * 10000
        sign = "+" if med_slip_bps > 0 else ""
        k6.metric("Median Slippage", f"{sign}{med_slip_bps:.2f} bp")
    else:
        k6.metric("Median Slippage", "—")
