...
EXPENSE_PER_CR   = float(os.getenv("EXPENSE_PER_CR", "0"))  # ₹10k per Cr traded
...
def load_data(book_filter: str = "all") -> list[dict]:
    ...
    """
    data = load_data_from_redis()           # <-- LIVE: from dashboard_worker via Redis
    if not data:
        data = DUMMY_DATA                   # <-- FALLBACK: dummy data if Redis empty
...
def load_data_from_redis() -> list[dict]:
    ...
    Redis DB 0 (dashboard output DB).
    ...
    try:
        ...
        # Store as_of in session state for display
...
def main():
    ...
    # Uses session state timer so expand/collapse state is preserved
    now = time.time()
    if st.session_state.first_load:
        # On first load force immediate rerun to get live data
        st.session_state.first_load = False
        st.session_state.last_refresh = now
        st.rerun()
    elif now - st.session_state.last_refresh > 10:
        st.session_state.last_refresh = now
        st.rerun()
    ...
    with col_time:
        ...
        if source == "log_file":
            src_html = "<span style='color:#2eca8a'>● LIVE</span>"
        else:
            src_html = "<span style='color:#e8a825'>● DUMMY DATA</span>"
        st.html(
