    ...
    for _, row in df.iterrows():
        if row["is_stock"]:
            ...
            <tr class="expiry">
              ...
              <td style="color:#2eca8a;text-align:right;padding-right:12px">{b_str}</td>
              <td style="color:#e85c5c;text-align:right;padding-right:12px">{s_str}</td>
              {lots_html}
              {neutral_td(row["net_exp"])}
              {neutral_td(row["traded_val"])}
              {pnl_td(row["carry"])}
...
def build_position_table_html(data: list[dict], expand_all: bool, expanded_syms: set) -> str:
    ...
    for item in data:
        ...
        <tr class="stock">
          ...
          <td style="color:#2eca8a;text-align:right;padding-right:12px">{latest_b_avg}</td>
          <td style="color:#e85c5c;text-align:right;padding-right:12px">{latest_s_avg}</td>
          {lots_td}
          <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(s_net_exp)}</td>
          <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(s_tval)}</td>
          <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(sum(x["cost"] for x in exp_calcs))}</td>
          <td style="color:#7a8294;text-align:right;padding-right:12px;font-size:10px">{f"{sum(x['cost_pct'] for x in exp_calcs if x['traded_val']):.4f}%" if any(x['traded_val'] for x in exp_calcs) else "—"}</td>
          {pnl_td(s_carry)}
          {pnl_td(s_day)}
          {pnl_td(s_net)}
          {stock_pct_td}
        ...
        if is_open:
            for ec in exp_calcs:
                ...
                <tr class="expiry">
                  ...
                  <td style="color:#e85c5c;text-align:right;padding-right:12px">{s_str}</td>
                  {lh}
                  <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(ec["net_exp"])}</td>
                  <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(ec["traded_val"])}</td>
                  <td style="color:#7a8294;text-align:right;padding-right:12px">{fmt_inr(ec["cost"])}</td>
                  <td style="color:#7a8294;text-align:right;padding-right:12px;font-size:10px">{f"{ec['cost_pct']:.4f}%" if ec["traded_val"] else "—"}</td>
                  {pnl_td(ec["carry"])}
                  {pnl_td(ec["day"])}
                  {pnl_td(ec["net"])}
                  {pnl_pct_td}
