...
def load_eod(token_map: dict) -> dict[int, dict]:
    ...
    Reads previous day log from remote SSH server.
    Falls back to DUMMY data if prev log not available.
    """
    prev_dt   = prev_trading_date()
    prev_path = log_file_path(prev_dt)

    log.info("Loading EOD from remote prev log: %s", prev_path)
    result = _eod_from_log(prev_path)

    if not result:
        log.warning("Prev remote log empty or not found (%s) — using DUMMY EOD data", prev_path)
